...
const Suka = ({navigation, route}) => {
  ...
  const onPressSuka = async id => {
    let token = await AsyncStorage.getItem('token');

    axios.delete(`${env.API_URL}/suka/${id}`, {
      headers: {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.get(
        `${env.API_URL}/suka?limit=${dataLimit}&search=${search}&user_id=${user_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
